...
const ManageDestak: React.FC = () => {
  ...
  const addDestaque = () => {
    if (nome && banner) {
      setDestaques([...destaques, { id: Date.now(), nome, banner, imagens: [] }]);
      setNome('');
      setBanner('');
    } else {
  ...
  const addImageToDestaque = (id: number, image: string) => {
    setDestaques(destaques.map(d => d.id === id ? { ...d, imagens: [...d.imagens, image] } : d));
  };

  const removeImageFromDestaque = (id: number, index: number) => {
    setDestaques(destaques.map(d => d.id === id ? { ...d, imagens: d.imagens.filter((_, i) => i !== index) } : d));
  };
