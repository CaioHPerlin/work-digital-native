  ...
  const fetchNotifications = async () => {
    try {
      ...
      const unreadCount = data.reduce((count, chat) => {
        ...
        if (isUnread) {
          unreadChatsSet.add(chat.id); // Track unread chat ids
        }

        return count + (isUnread ? 1 : 0);
      }, 0);

      // Only update state if the unread chat count has changed
      setUnreadChats((prev) => {
      ...
      // Only update notification count if it has changed
